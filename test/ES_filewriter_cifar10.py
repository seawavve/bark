...
\n''')
    ...
    f.write("filename=\'checkpoint.h5\'.format("+str(epoch)+")\n")
    f.write("early_stopping=EarlyStopping(monitor='val_loss',mode='min',patience=15,verbose=1)\n")
    f.write("checkpoint=ModelCheckpoint(filename,monitor='val_loss',verbose=1,save_best_only=True)\n")
    ...
    # !
    f.write("inputs = keras.Input(shape = x_train.shape[1:], name = 'input')\n")
...
for _ in range(depth):
  ...
  if (short_cut_dec<=byp) and len(net_list)>1:
    ...
    f.write("hist = model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_data=(x_test, y_test),callbacks=[checkpoint,early_stopping])\n\n")
